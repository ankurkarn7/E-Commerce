...
const AcceptedSellerOrders = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try{
        const url =
            user?.role === 'user'
            ? "/order/userAcceptedOrders"
            : "/order/sellerAcceptedOrders";
  ...
  return (
    <div>
      <div className='sticky top-[57px] w-full flex flex-wrap gap-3 justify-between items-center px-6 py-4 z-10 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm'>
            <h1 className='font-extrabold text-2xl text-slate-800'>Accepted Orders</h1>
            <MenuBox />
        </div>
         <div className='max-w-6xl mx-auto p-6'>
            {loading ? (
                ...
                <p className='text-slate-500 text-xl mt-20 flex justify-center'>No accepted orders.</p>
            )
        }
      </div>
...
export default AcceptedSellerOrders
