  ...
  return (
    <div>
      <div className='sticky top-[57px] w-full flex flex-wrap gap-3 justify-between items-center px-6 py-4 z-10 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm'>
            <h1 className='font-extrabold text-2xl text-slate-800'>Cancelled Orders</h1>
            <MenuBox />
        </div>
         <div className='max-w-6xl mx-auto p-6'>
            {loading ? (
                ...
                {products.map((p) => (
                    <OrderCard key={p._id} product={p} />
                ))}
                </div>
            ) : (
                <p className='text-slate-500 text-xl mt-20 flex justify-center'>No cancelled orders.</p>
            )
        }
      </div>
